...
import { ChevronRight, Clock3, MapPin, Pencil, ReceiptText, Trash2 } from 'lucide-react'
import Modal from './Modal'
import DeliveryFormModal from './DeliveryFormModal'
import { formatCurrency, formatDateTime, formatDistance } from '../utils/formatters'

export default function DeliveryHistory({ deliveries, onEdit, onDelete }) {
    ...
    <>
      ...
      <Modal open={Boolean(selected)} onClose={() => setSelected(null)} title="Detalhes da entrega">
        {selected && <div className="modal-body delivery-details">
          ...
          {canEdit(selected) ? <div className="detail-actions"><button className="button button--ghost" type="button" onClick={() => { setEditing(selected); setSelected(null) }}><Pencil size={17} /> Corrigir</button><button className="button button--danger" type="button" onClick={async () => { await onDelete(selected); setSelected(null) }}><Trash2 size={17} /> Excluir</button></div> : <div className="edit-expired"><Clock3 size={16} /> O prazo de 10 minutos para correção ou exclusão terminou. Procure um administrador.</div>}
        </div>}
      </Modal>
