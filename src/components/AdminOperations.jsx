...
import { adminCancelDelivery, adminUpdateDelivery, summarizeDeliveries } from '../services/deliveryService'
...
export default function AdminOperations({ profiles }) {
  ...
  const cancelDelivery = async (delivery) => {
    if (!window.confirm('Cancelar logicamente esta entrega?')) return
    try { await adminCancelDelivery(delivery.id); setEditing(null); setSelectedOrder(null); showToast('Entrega cancelada.', 'info'); await load() } catch (error) { showToast(error.message, 'error') }
  }
  ...
  return <div className="operations">
    ...
    <AdminDeliveryEditor open={Boolean(editing)} delivery={editing} onClose={() => setEditing(null)} onSave={saveDelivery} onCancelDelivery={cancelDelivery} />
  </div>
}
